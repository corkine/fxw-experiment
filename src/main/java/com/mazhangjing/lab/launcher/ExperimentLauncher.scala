  ...
  override def init(): Unit = {
    val stream =
      new FileInputStream(new File(
        if (!USE_CLASSPATH) System.getProperty("user.dir") + File.separator + "launch.yaml"
        else getClass.getClassLoader.getResource("lab" + File.separator + "launch.yaml").toString
    ))
